...
- `.codex/agents/*.toml` files are generated from the portable `agents/*.md` source through `bin/sync_codex_agents.py`; `--check` detects drift.
...
- `python3 -m unittest discover -s tests -p 'test_*.py'`: 7 passed at the time of the adaptation.
